...
The study supports a bounded empirical claim: adaptive fuzzy correction improves one-step Ghana cocoa yield forecasting relative to strong baseline references in this dataset, and fuzzy state-transition models are especially useful during high-volatility years. The sample is small, so paired tests are reported as descriptive evidence rather than confirmatory proof of universal model dominance.

## License

A license should be added before public release. If no license is provided, reuse permissions are unclear for external users.
